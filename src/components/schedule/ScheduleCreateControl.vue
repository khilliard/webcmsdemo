  ...
  <div>
    <v-dialog v-model="createPopup" max-width="600">
      <template v-slot:activator="{ on }">
        ...
        <span v-if="schedule" style="display: block;" @click="initPopup" v-on="on">
          <slot>
      ...
      <v-card outlined>
        <v-card-title class="primary mb-2">{{ schedule ? "Update" : "Create" }} Schedule</v-card-title>
        <v-card-text>
    ...
    <v-dialog v-model="confirmPopup" max-width="250">
      <v-card outline>
        ...
        <v-card-text>
          <v-form>
            <v-btn color="red" :loading="waiting" :disabled="waiting" @click="deleteSchedule">Yes</v-btn>
      ...
      try {
        ...
        this.players.forEach(player => {
          ...
          if (data.schedule == schedulePath) {
            console.log(
              `removing assigned schedule from player '${data.name}'`
            );
            // undo assignment
            data.schedule = null;
